...
**关于 token 层的说明**：本规范的双主题 token 为**独立设计**，不依赖任何第三方设计系统的数值，可整体迁移或直接用于新项目。如需更换品牌色，只需覆盖 `--color-primary` 及其 hover / active / subtle 四处（见 §2.7）；**不要改动语义色的结构**——语义色一旦增删，组件规格与状态映射会全部失效。
