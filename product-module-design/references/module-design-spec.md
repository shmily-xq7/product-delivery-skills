...
根据当前开发的功能模块，从产品设计文档（`项目战术执行/00_产品设计文档.md`，相对于目标项目根目录）的"功能设计详情"章节中，**完整复制**对应功能或页面的设计内容。
...
- **验收标准**：各页面/弹窗的 `验收标准` 小节（含 `AC-<模块缩写>-<两位序号>` 编号），原样复制

### 本模块覆盖的 AC

复制完成后，在「产品设计」这部分的**开头**补充一张清单，列出本模块实现的全部验收标准编号：
