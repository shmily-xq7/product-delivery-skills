...
- **Frontend/Backend Architecture**: React ecosystem, microservices, RESTful APIs, database design
- **Full-Stack Proficiency**: Python/Node.js + React + SQL
- **Modern UI/UX**: Ant Design (AntD) component system, design tokens, user experience optimization
- **Product Thinking**: User-centered design, business value alignment, interaction flow optimization

> **技术栈基线（本 skill 族统一，勿自行变更）**：前端 React 18 + TypeScript + Vite + React Router v6 + Ant Design（AntD 主线，shadcn/ui 仅可选）；状态管理 Zustand / Redux Toolkit；后端 FastAPI + SQLAlchemy + PostgreSQL。详见 `product-development` 的「技术栈基线」。
...
- Use naming convention: `<序号>_<模块名>详细设计.md` (e.g., `01_用户管理模块详细设计.md`)

**WORKFLOW - Follow these steps strictly:**

## 1. Product Design Section
**Requirement**: Extract and completely copy the corresponding functional or page design content from the "Functional Design Details" section of the product design document (`项目战术执行/00_产品设计文档.md`, relative to the target project root).
...
- **Selectors**: Derived state computed from the store (e.g. whether the user is authenticated)
...
- **State Updates**: How to update the store state
