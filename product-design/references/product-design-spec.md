...
**多步骤弹窗设计要求**：
- 每个步骤都必须输出：步骤布局图 + 步骤概要 + 交互矩阵（必选） + 字段字典（按需） + 验收标准（必选）
...
**子tab页设计要求**：
- 每个子tab页都必须输出：tab页布局图 + tab页概要 + 交互矩阵（必选） + 字段字典（按需） + 验收标准（必选）
...
- **验收标准必写且用列表**：每个页面/弹窗/步骤/tab 都要有`验收标准`小节，条目带全文唯一的 AC 编号；**不要**为它另开表格（否则突破上一条约束）
...
每个页面、弹窗、抽屉、多步骤弹窗的每一步、子 tab 页都必须给出**验收标准**。它是设计文档从"描述"变成"可验收"的凭据，也是下游 E2E 测试用例的引用锚点。

### 6.1 编号规则

格式：`AC-<模块缩写>-<两位序号>`

- **模块缩写**：2–8 位大写字母或数字，取模块/页面的英文缩写，同一模块内保持一致（如 `USER`、`PROJ`、`ORDER`）
- **两位序号**：从 `01` 起，两位数字
- 完整示例：`AC-USER-01`、`AC-PROJ-03`
- **全文唯一**：同一个编号不得在文档里出现两次
- 脚本识别用正则：`AC-[A-Z][A-Z0-9]{1,7}-[0-9]{2}`

采用纯 ASCII 编号，是因为编号要出现在 E2E 测试代码的注释里、还要参与全局唯一性校验——中文编号在这两处都不可靠。

### 6.2 写在哪里

| 层级 | 位置 | 覆盖范围 |
|---|---|---|
| 页面 | 页面小节末尾，标题 `验收标准` | 页面主体：列表默认排序、查询、分页、行级操作、空态/加载/错误态 |
| 弹窗 / 抽屉 / 步骤 / 子 tab | 该元素自己的小节内，标题 `验收标准` | 该元素自身：打开与关闭、实时校验、提交、成功后列表刷新 |

**用编号列表，不要用表格。** 「每个页面/弹窗 ≤ 2 张表」是硬约束，验收标准另开一张表会直接违反它。

### 6.3 单条怎么写

每条 = **编号 + 一句可判定的描述**，描述尽量写成「条件 → 操作 → 预期结果」：

```
- **AC-USER-01** 进入页面：默认按创建时间倒序，分页 20 条/页；无数据时展示空态「暂无用户」
- **AC-USER-02** 查询：用户名支持模糊匹配；命中 0 条时列表切换空态并保留搜索词
- **AC-USER-03** 删除：二次确认后执行软删除；列表立即移除该行，刷新后仍不出现
```
...
```
#### 验收标准
- **AC-<模块缩写>-01** <条件> → <操作> → <预期结果>
- **AC-<模块缩写>-02** <条件> → <操作> → <预期结果>
```

**判定门槛**：一条 AC 必须能被写成一个测试断言。写不成断言的（例如「界面美观」「性能良好」）说明它还没写到位，应拆成可观测的结果。

### 6.4 下游怎么用

- **模块详细设计**（`product-module-design`）：列出「本模块覆盖的 AC」，说明本模块实现了哪些编号
...
- **验收标准（必选）**：该弹窗自身的验收条目（打开/关闭、实时校验、提交、成功后列表刷新），编号接入全文 AC 序列
...
### 6. 验收标准
包含：
- **编号列表（必选）**：每条以 `AC-<模块缩写>-<两位序号>` 开头，覆盖页面主体的关键路径与异常分支
- 覆盖范围：列表默认排序与分页、查询/重置、行级操作、页面级操作、空态/加载/错误态
- 弹窗、抽屉、多步骤弹窗的每一步、子 tab 页，各自在自己的小节内另附「验收标准」

#### 验收标准填写要求
- 每条用「条件 → 操作 → 预期结果」写法，**能被直接翻译成一个测试断言**
- 编号全文唯一，不得跨页面重复
- **不要**用表格承载验收标准——会突破「每页 ≤2 张表」的约束，用编号列表
- 完整编号规则与下游用法见第 6 节「验收标准规范」
...
- 数据流清晰：输入输出是否明确
- 设计一致性：界面风格是否统一
- 验收标准完备：每个页面/弹窗/步骤/tab 是否都有 AC 条目；编号是否全文唯一；每条是否可判定为测试断言
...
- **AC-USER-11** 打开弹窗：字段初始化为默认值，焦点落在「用户名」；部门与角色下拉数据来自接口，加载失败时提示且不展示弹窗
- **AC-USER-12** 实时校验：用户名重复时失焦后提示「用户名已存在」，「确认」按钮置灰；错误提示消失后按钮恢复可用
- **AC-USER-13** 提交成功：点击「确认」后按钮进入 loading；成功后弹窗关闭，列表刷新并出现新增行
- **AC-USER-14** 提交失败：接口报错时提示具体失败原因；弹窗保持打开且已填内容不丢失

#### 验收标准

- **AC-USER-01** 进入页面：默认按创建时间倒序，分页 20 条/页；无数据时展示空态「暂无用户」
- **AC-USER-02** 查询：用户名支持模糊匹配，状态与角色精确匹配；点击「重置」清空全部条件并回到默认排序
- **AC-USER-03** 分页与排序：切换每页条数或页码后列表刷新；点击表头「创建时间」在升序与降序之间切换
- **AC-USER-04** 行级操作：编辑、删除、分配角色均只作用于当前行；删除需二次确认，确认后该行从列表移除
- **AC-USER-05** 异常态：列表接口失败时展示错误提示与「重试」；加载中展示骨架，不出现空白闪烁
...
| 验收标准完备       | ✅ 通过  | 页面与弹窗均有 AC 条目，编号全文唯一，每条均可转为测试断言   |
