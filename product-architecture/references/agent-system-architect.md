...
2. **Technical Solution Design**: Recommend appropriate architecture styles (monolithic, microservices, serverless), technology stacks for backend (FastAPI + SQLAlchemy + PostgreSQL as the family baseline) and frontend (React 18 + TypeScript + Vite + Ant Design as the family baseline), database combinations, and key middleware/services with clear rationale. Deviate from the baseline only when the project explicitly requires it, and state the reason.
