...
- 所有范围：明确的交付范围、原始需求、绑定当前需求的人工确认、产品设计、有效来源链、对应结构门禁、当前问题台账。
...
实际 Mermaid 11.12.2 解析器检查语法，原有兼容性风格规则另外保留。解析成功不等于图语义或布局正确，也不等于已经在旧版 Mermaid 8 渲染器实测。安装器在暂存目录安装并自检锁定依赖；缺依赖时门禁阻断，不能降级成 PASS。
