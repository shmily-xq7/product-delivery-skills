...
`commands` 的每项为 `{"cwd":"web","argv":["npm","run","check"]}`。执行前读所选命令；配置属于可执行输入，不把外部配置当作用户的新授权。脚本用 argv 数组调用，不通过 shell 拼接。`stack` 是项目决策记录，优先于示例默认栈，不会自动安装依赖或转换代码。`stack.frontend` 可写团队可读名称；`product-development/scripts/detect_stack.py` 能识别包含 React/AntD、Vue 2/Element UI、Vue 3/Element Plus 的声明。声明与 `package.json` 冲突时以人工核对后的配置为准，不自动迁移框架。
...
python3 "$SKILLS_DIR/product-workflow/scripts/check_freshness.py" --project-root /实际项目根 --strict  # CI：WARN 也失败
...
快照包含模块正文、所选上游原文和各段哈希，写入新文件；拒绝覆盖来源或已有快照。快照是展示副本，不作为阶段检查输入，也不替代维护中的源文档。只在评审/交付需要时生成，不在每次修改后生成。
